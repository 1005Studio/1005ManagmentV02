...
const App: React.FC = () => {
  ...
  const [isCalendarOpen, setIsCalendarOpen] = useState(false); // NEW STATE
  ...
  useEffect(() => {
    // Only listen to DB if user is logged in
    if (!user) return;
  ...
  const pendingInvoiceCount = useMemo(() => filteredVideos.filter(v => !v.isInvoiced && v.status === VideoStatus.COMPLETED && v.status !== VideoStatus.CANCELLED && v.status !== VideoStatus.REPEAT).length, [filteredVideos]);
    ...
    return (
      <div className="min-h-screen bg-[#F8F9FA] dark:bg-[#050505] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 animate-pulse">
           <div className="w-16 h-16 bg-[#1A1A1A] dark:bg-white rounded-2xl flex items-center justify-center shadow-xl">
             <StudioLogo className="w-10 h-10" />
           </div>
           <div className="h-2 w-24 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
              <div className="h-full bg-[#D81B2D] animate-slide-up w-1/2"></div>
           </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] dark:bg-[#050505] pb-32 font-body text-[#1A1A1A] dark:text-gray-100 print:bg-white print:pb-0 transition-colors duration-300 animate-fade-in">
              ...
              <div className="flex justify-between items-center h-16 md:h-20">
                ...
                <div className="hidden md:flex items-center gap-4">
                    ...
                    <div className="flex items-center p-1.5 bg-gray-100/50 dark:bg-[#1E1E1E]/50 rounded-full border border-gray-200/50 dark:border-gray-700/50">
                        <button onClick={() => setIsCalendarOpen(true)} className="p-2.5 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-white dark:hover:bg-[#2C2C2C] rounded-full transition-all" title="Takvim">
                            <CalendarIcon />
                        </button>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {/* Logout Button */}
                        <button onClick={handleLogout} className="p-2.5 rounded-full text-gray-500 dark:text-gray-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-all" title="Çıkış Yap">
                           <LogOutIcon />
                        </button>
                    </div>
                ...
                <div className="flex md:hidden w-full items-center gap-2 ml-4">
                   ...
                   <button onClick={() => setIsCalendarOpen(true)} className="p-2 rounded-full text-gray-600 dark:text-gray-300">
                      <CalendarIcon />
                   </button>
                   ...
                   <button onClick={handleLogout} className="p-2 rounded-full text-gray-600 dark:text-gray-300 hover:text-red-500">
                      <LogOutIcon />
                   </button>
                </div>
              </div>
      ...
      <CalendarModal isOpen={isCalendarOpen} onClose={() => setIsCalendarOpen(false)} videos={videos} />
    </div>
  );
};
