      ...
      <div className="bg-[#F8F9FA] rounded-2xl w-full max-w-6xl overflow-hidden flex flex-col h-[90vh] shadow-2xl border border-gray-800">
        <div className="flex justify-between items-center p-5 border-b border-gray-200 bg-white">
          <div>
            <h2 className="font-extrabold text-2xl text-gray-900 font-brand flex items-center gap-3">
              <span className="bg-[#D81B2D] text-white p-1.5 rounded-lg"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg></span>
              Bu Cuma (Styling)
            </h2>
            <p className="text-sm text-gray-500 font-medium mt-1 ml-1">Çekim referans görselleri ve ürün hazırlığı.</p>
          </div>
        ...
        <div className="p-8 overflow-y-auto flex-1">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 border-4 border-dashed border-gray-200 rounded-3xl bg-white m-4">
              ...
              <p className="font-bold text-xl text-gray-900">Görsel Hazinesi Boş</p>
              <p className="text-base mt-2 text-gray-500 max-w-md">Çekim için styling referanslarını veya ürün görsellerini buraya yükleyerek başlayın.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {items.map(item => (
                <div key={item.id} className={`group relative aspect-square bg-white rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.08)] border overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ${item.isCompleted ? 'border-green-500 ring-4 ring-green-500/10' : 'border-gray-100'}`}>
                  <img src={item.imageUrl} alt="Styling Ref" className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${item.isCompleted ? 'grayscale-[80%] opacity-80' : ''}`} />
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all flex flex-col justify-end p-4">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        className={`flex-1 py-2 rounded-lg shadow-lg text-xs font-bold pointer-events-auto z-10 transition-transform active:scale-95 ${item.isCompleted ? 'bg-yellow-500 text-white' : 'bg-green-600 text-white'}`}
                        >
                        {item.isCompleted ? "GERİ AL" : "TAMAMLA"}
                        </button>
                        <button 
                        onClick={(e) => { e.stopPropagation(); onDelete(item.id); }}
                        className="w-9 h-9 flex items-center justify-center bg-white text-red-600 rounded-lg hover:bg-red-50 pointer-events-auto z-10 shadow-lg transition-transform active:scale-95"
                        title="Resmi Sil"
                        >
                  ...
                  {/* Status Indicator */}
                  {item.isCompleted && (
                    <div className="absolute top-3 right-3 bg-green-500 text-white text-[10px] font-bold px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1.5 z-20 tracking-wide">
        ...
        <div className="p-6 border-t border-gray-200 bg-white flex justify-between items-center gap-6 shadow-[0_-4px_30px_rgba(0,0,0,0.03)] relative z-20">
          ...
          <div className="text-sm text-gray-500 font-medium">
             <strong className="text-gray-900">{items.length}</strong> görsel yüklendi
          </div>
          <button 
